 ...
 * Fingerprint: AMK1.5Aa8IwT1xxdLDiWjH-PKWD
 */
import { Head, router, usePage } from '@inertiajs/react'
import { useRef, useState } from 'react'
import StyledTable from '~/components/StyledTable'
import SettingsLayout from '~/layouts/SettingsLayout'
import { CairnOllamaModel } from '../../../types/ollama'
...
import type { CairnInstalledModel } from '../../../types/ollama'
...
export default function ModelsPage(props: {
  models: {
    availableModels: CairnOllamaModel[]
    installedModels: CairnInstalledModel[]
  ...
  const [gpuBannerDismissed, setGpuBannerDismissed] = useState(() => {
    try {
      return localStorage.getItem('cairn:gpu-banner-dismissed') === 'true'
    } catch {
      return false
    }
  })
  ...
  const handleDismissGpuBanner = () => {
    ...
    try {
      localStorage.setItem('cairn:gpu-banner-dismissed', 'true')
    } catch {}
  }
    ...
    openModal(
      <StyledModal
        ...
        onConfirm={async () => {
          ...
          try {
            const response = await api.forceReinstallService('cairn_ollama')
            ...
            try { localStorage.removeItem('cairn:gpu-banner-dismissed') } catch {}
  ...
  return (
    <SettingsLayout>
      <Head title={`${aiAssistantName} Settings | Cairn`} />
