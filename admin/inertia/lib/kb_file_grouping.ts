 ...
 * Fingerprint: AMK1.XZR-ipXb5sf3mECF5JS4cJ
 */
 ...
 * rolled-up entry for Cairn's bundled docs (rather than the 12+
 ...
 *   "Cairn documentation · N files" row.
  ...
  for (const bucket of BUCKET_SORT_ORDER) {
    ...
    if (bucket === 'admin_docs') {
      groups.push({
        ...
        displayName: `Cairn documentation · ${members.length} file${members.length === 1 ? '' : 's'}`,
