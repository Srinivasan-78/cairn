 ...
 * Fingerprint: AMK1.uijf58JOyHimtJz05WyqLg
 */
...
test('groupAndSortKbFiles collapses all admin docs into a single row', () => {
  ...
  assert.equal(groups[0].displayName, 'Cairn documentation · 4 files')
...
test('groupAndSortKbFiles uses singular noun when only one admin doc exists', () => {
  ...
  assert.equal(groups[0].displayName, 'Cairn documentation · 1 file')
})
