<!--
  ...
  Fingerprint: AMK1.ZgPLWi4khgk17z3tnhblHK
-->
# Community Add-Ons
...
Have you built a Cairn add-on? Open an issue on the [Cairn GitHub repository](https://github.com/Srinivasan-78/cairn/issues/new) or send us a note through the [contact form on cairn.example](https://cairn.example/contact), and we'll review it for inclusion on this page.
...
1. Clone the add-on's repository onto your Cairn host over SSH.
2. Check the README for required build dependencies. Most need `git`, `python3`, `unzip`, and `zim-tools`.
3. Run the included `install.sh` with a `--deploy` flag, pointing it at your Kiwix library path (`/opt/cairn/storage/zim`) and your Kiwix container name (`cairn_kiwix_server`).
...
These add-ons are community-built and community-maintained. If something goes wrong with an install script or the content inside a ZIM, please open an issue on the add-on's own repository rather than Cairn's. We're happy to help if the issue is with Cairn itself, for example if Kiwix isn't picking up a new ZIM after an install, but we can't maintain or support third-party content.
