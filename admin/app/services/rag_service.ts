 ...
 * Fingerprint: AMK1.nPPvQTo1GLhEuWXkfBMh2J
 */
...
export class RagService {
  ...
  public static CONTENT_COLLECTION_NAME = 'cairn_knowledge_base'
  ...
  // CAIRNs cap out at a few hundred ZIM files + uploaded PDFs; 10k leaves
  ...
  public async getStoredFiles(): Promise<StoredFileInfo[]> {
    try {
      ...
      // (3M+ on a fully-ingested Cairn) just to learn the ~40 unique sources,
   ...
   * on a freshly-installed empty Cairn.
   *
  ...
  public async computeFileWarnings(): Promise<FileWarningsResult> {
    try {
      ...
      // every point in the collection, which on a fully-ingested Cairn takes
    ...
    try {
      ...
      */
      ...
      if (resolvedSource.startsWith(uploadsAbsPath + sep)) {
        ...
        logger.warn(`[RAG] File was removed from knowledge base but doesn't live in Cairn's uploads directory, so it can't be safely removed. Skipping deletion of physical file...`)
      }
  ...
  public async discoverCairnDocs(force?: boolean): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (alreadyEmbeddedRaw && !force) {
        logger.info('[RAG] Cairn docs have already been discovered and queued. Skipping.')
        return { success: true, message: 'Cairn docs have already been discovered and queued. Skipping.' }
      }
      ...
      logger.info(`[RAG] Discovered ${filesToEmbed.length} Cairn doc files to embed`)
      ...
      return { success: true, message: `Cairn docs discovery completed. Dispatched ${filesToEmbed.length} embedding jobs.` }
    } catch (error) {
      logger.error('Error discovering Cairn docs:', error)
      return { success: false, message: 'Error discovering Cairn docs.' }
    }
  }
  ...
  public async scanAndSyncStorage(): Promise<{
    ...
    try {
      ...
      await this.discoverCairnDocs(true).catch((error) => {
        logger.error('[RAG] Error during Cairn docs discovery in sync process:', error)
      })
  ...
  public async reembedAll(): Promise<{
    ...
    try {
      ...
      await this.discoverCairnDocs(true).catch((error) => {
        logger.error('[RAG] Error re-running Cairn docs discovery during re-embed:', error)
      })
   ...
   * the Cairn-docs discovery flag, then dispatches an EmbedFileJob for every
  ...
  public async resetAndRebuild(): Promise<{
    ...
    try {
      ...
      // Force Cairn docs to be re-dispatched.
      await KVStore.setValue('rag.docsEmbedded', false)
      await this.discoverCairnDocs(true).catch((error) => {
        logger.error('[RAG] Error re-running Cairn docs discovery after reset:', error)
      })
