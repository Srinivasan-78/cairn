...
# Fingerprint: AMK1.QMhaIftw25neU2qFHPLAJ_

# Cairn — Disk Collector Migration Script
#
# Script                | Cairn Disk Collector Migration Script
# Version               | 1.0.0
...
#   process that wrote to /tmp/cairn-disk-info.json, which was bind-mounted
...
#   /opt/cairn/storage/cairn-disk-info.json, which the admin container
...
CAIRN_DIR="/opt/cairn"
COMPOSE_FILE="${CAIRN_DIR}/compose.yml"
...
check_confirmation() {
  echo -e "${YELLOW}#${RESET} This script migrates your Cairn installation from the"
  ...
  echo -e "${RED}#${RESET} STOP: If you have customized your compose.yml or Cairn's storage setup (not common), please make these changes manually instead of using this script!\n"
...
check_compose_file() {
  if [[ ! -f "$COMPOSE_FILE" ]]; then
    ...
    echo -e "${RED}#${RESET} Cairn does not appear to be installed or compose.yml is missing."
    exit 1
  fi
...
stop_old_host_process() {
  local pid_file="${CAIRN_DIR}/cairn-collect-disk-info.pid"
...
remove_old_bind_mount() {
  if ! grep -q 'cairn-disk-info\.json' "$COMPOSE_FILE"; then
    echo -e "${GREEN}#${RESET} Old /tmp/cairn-disk-info.json bind-mount not found — already removed.\n"
    return 0
  fi

  echo -e "${YELLOW}#${RESET} Removing old /tmp/cairn-disk-info.json bind-mount from admin volumes..."
  sed -i '/\/tmp\/cairn-disk-info\.json:\/app\/storage\/cairn-disk-info\.json/d' "$COMPOSE_FILE"

  if grep -q 'cairn-disk-info\.json' "$COMPOSE_FILE"; then
    echo -e "${RED}#${RESET} Failed to remove old bind-mount from compose.yml. Please remove it manually:"
    echo -e "${WHITE_R}      - /tmp/cairn-disk-info.json:/app/storage/cairn-disk-info.json${RESET}"
    exit 1
  fi
...
add_disk_collector_service() {
  ...
  awk '/^volumes:/{
    ...
    print "    image: ghcr.io/srinivasan-78/cairn-disk-collector:latest"
    print "    pull_policy: always"
    print "    container_name: cairn_disk_collector"
    print "    restart: unless-stopped"
    print "    volumes:"
    print "      - /:/host:ro,rslave  # Read-only view of host FS with rslave propagation so /sys and /proc submounts are visible"
    print "      - /opt/cairn/storage:/storage  # Shared storage dir — disk info written here is read by the admin container"
    print ""
  }
...
verify_disk_collector_running() {
  sleep 3
  if docker ps --filter "name=^cairn_disk_collector$" --filter "status=running" --format '{{.Names}}' | grep -qx "cairn_disk_collector"; then
    ...
    echo -e "${RED}#${RESET} Check its logs with: docker logs cairn_disk_collector"
    exit 1
  fi
}
...
# Main
echo -e "${GREEN}#########################################################################${RESET}"
echo -e "${GREEN}#${RESET}      Cairn — Disk Collector Migration Script             ${GREEN}#${RESET}"
echo -e "${GREEN}#########################################################################${RESET}\n"
