<!--
  ...
  Fingerprint: AMK1.3cGp06VR7gZjwUrmyIil2p
-->
...
### Why the Migration?
The new disk-collector sidecar provides a more robust and scalable way to collect disk information from the host. It removes the original bind mount to `/tmp/cairn-disk-info.json`, which was fragile and prone to issues on host reboots.
...
- The bind mount to `/tmp/cairn-disk-info.json` was cleared on host reboots, causing Docker to create a directory at the mount point instead of a file.
...
### Why does Cairn need the cairn-disk-info.json file?
Cairn uses the disk info stored and updated in `cairn-disk-info.json` to allow users to view disk usage and availability within the Cairn "Command Center". While not critical to the core functionality of Cairn, it provides a more pleasant experience for users with limited storage space and/or who aren't familiar with command-line tools and Linux management.
